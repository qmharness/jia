...
| type | 字段 | 说明 | 响应 |
|---|---|---|---|
| `hello` | `cwd` | 解析工作区:已有 `.jia/config.toml` 直接读取;否则(家目录、`/`、`/tmp`、`/usr*` 除外)先向客户端发 `confirm_request`(tool=`jia_init`)询问是否建工作区 | `workspace_resolved` |
| `agent` | `messages`(`[{role, content}]`)、`session_id`(可空,空则新建)、`cwd`、`workspace_id` | 发起一次 agent 运行 | 先回 `session`,随后流式事件,以 `done` 结束 |
