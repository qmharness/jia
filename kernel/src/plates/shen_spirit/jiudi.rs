...
impl Hook for JiudiHook {
    fn name(&self) -> &str { "jiudi" }
    fn spirit_types(&self) -> Vec<SpiritType> { vec![SpiritType::JiuDi] }

    async fn on_event(&self, event: HookEvent) -> HookResult {
        if let HookEvent::CompactionTriggered { messages_before, messages_after, tokens_before, tokens_after, method } = &event {
            tracing::info!(
