...
The Heaven Plate is the system's "runtime." It is the only perspective that runs main-turn inference (main_core, through the `pub(crate)`-encapsulated `JiaCore::infer`). Auxiliary LLM calls (aux_core) — vijnana/vasana distillation, li_skill reflection and revision, delegate subagents, xun_context compression — live outside the Heaven Plate as the documented exceptions recorded in §9; the gateway `/chat` endpoint is a raw LLM passthrough (no agent loop, no GeJu), likewise a documented exception. Each Heaven Plate turn corresponds to one "temporal hour" (时辰) in Qimen Dunjia—the stems rotate by one palace.
...
Jia (the LLM) is only indirectly accessible through the Six Ceremonies (tool classification) and GeJu (safety evaluation). Main-turn inference runs only in the Heaven Plate loop (main_core); the auxiliary LLM call sites (aux_core: vijnana/vasana distillation, li_skill reflection and revision, delegate subagents, xun_context compression) and the gateway `/chat` raw passthrough are the documented exceptions recorded in §9. This ensures every main-turn LLM call passes through safety policy evaluation.
